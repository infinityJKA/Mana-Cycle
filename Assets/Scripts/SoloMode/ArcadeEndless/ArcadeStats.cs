...
public class ArcadeStats
{
    ...
    public static string StatToString(Stat s)
    {
        if(LocalizationSettings.SelectedLocale == LocalizationSettings.AvailableLocales.GetLocale("ja")){
            switch(s)
            {
                case Stat.DamageMult: return "ダメージ倍率";
                case Stat.StartingSpecial: return "始まりのスペシャル";
                case Stat.SpecialGainMult: return "スペシャル倍率";
                case Stat.StartingCycleModifier: return "始まりのサイクルボーナス";
                case Stat.CycleMultIncrease: return "フルサイクルボーナス";
                case Stat.QuickDropSpeed: return "手動ドロップ速度";
                case Stat.MoneyMult: return "お金倍率";
                default: return "この統計は存在しません";
            }
        }
        else{
            switch(s)
